...
class BigtechDeskTheme {
	...
	_boot() {
		...
		const bootData = window.frappe?.boot?.bigtech_theme;
	...
	_fetchSync() {
		try {
			...
			xhr.open("GET", "/api/method/bigtech_theme.api.get_bigtech_theme", false); // false = sync
	...
	async _fetch(force) {
		...
		try {
			const r = await frappe.xcall("bigtech_theme.api.get_bigtech_theme");
			if (r) {
				this.themeData = r;
				this._writeCache(r);
				this._apply();
	...
	_setVars() {
		...
		set("--bt-sidebar-badge-color", t.sidebar_badge_color);
...
(function () {
	function init() {
		if (window.BigtechDeskTheme) return;
	...
	document.addEventListener("boot_info_ready", () => {
		if (window.BigtechDeskTheme) {
			window.BigtechDeskTheme._boot();
		}
	});
})();
